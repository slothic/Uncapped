...
-- [MQ-05] THIS FILE IS THE ALWAYS-LOADED HALF, AND IT IS DELIBERATELY SMALL.
--
-- It used to carry UncappedQuestGivers as well -- 7,461 rows, 640 KB of the
-- file's 708 KB -- and because this is the FIRST entry in UncappedQuests.toc,
-- every player parsed and materialised all of it at every login and every
-- /reload. That is roughly 2 MB of permanent Lua heap, paid by everyone, for a
-- table only the OPT-IN pick-up arrow and giver pins ever read (db.availEnabled
-- and db.showGivers, both default false).
--
-- The giver tables now live in the UncappedQuestData addon, which is
-- ## LoadOnDemand: 1 and is pulled in by UQ.EnsureQuestData() the first time
-- something actually needs them.
--
-- UncappedMapAreas STAYS HERE and must not be moved: the map pins, the waypoint
-- arrow and UQ.MapPos all read it unconditionally, so it is not optional and
-- deferring it would only trade a small cost for a load on the hot path.
--
-- UncappedZoneNames also stays. Its only reader is the ledger sidebar, which is
-- a core feature rather than an opt-in one, so deferring it would either mean a
-- second on-demand addon for 8% of the file or make opening the ledger pull in
-- the whole 640 KB giver table it has no use for.
