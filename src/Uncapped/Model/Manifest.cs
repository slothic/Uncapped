...
public sealed class ItemCacheSpec
{
    ...
    /// Size of the gzip.
    ///
    /// ⚠ LA-14. NOTHING READS THIS, and the comment here used to claim it was "for the log line
    /// and a sanity check" — which read as a guarantee that a wrong-sized download would be
    /// caught. It would not be, by this. <see cref="Sha256"/> is what rejects the gzip and
    /// <see cref="ContentSize"/> is what <see cref="WdbCache"/> checks the decompressed file
    /// against. Kept because it is genuinely useful when reading the published manifest by
    /// hand; do not add a check that depends on it without saying so here.
    /// </summary>
...
/// <summary>
/// One archive of a multi-part HTTP client download.
///
/// ★★ THE SHA256 IS NOT OPTIONAL, AND THIS IS THE ONE PLACE IN THE TREE WHERE THAT HAD TO
/// BE SAID OUT LOUD.
///
/// Until 2026-08-22 this type carried a url, a name and a byte count and no hash at all,
/// which made the 2.6 GB stock-client zip the only external input the launcher accepted
/// unverified — every other one is pinned: payload files, the MPQs, UncappedCT.dll,
/// ArkInventory's zip, the launcher exe, the item cache twice over. A byte count is not a
/// check. Anything able to choose the bytes can choose the length as well.
///
/// It matters more here than anywhere else because of WHAT this download is: the mechanism
/// by which a new player's game files arrive, expanded into their install root before
/// anything has had a chance to disagree with it. <see cref="IntegrityVerifier"/> is a
/// weaker backstop than it looks — it walks Data\ and root-level .exe/.dll only, every
/// finding it makes about an unexpected file is NON-BLOCKING, and the baseline it judges
/// against is itself fetched over plain http with no pin.
///
/// ⚠ AND THE TRANSPORT CANNOT CARRY THIS. The patch host listens on port 80 only and the
/// URL is a bare IP, which cannot present a certificate; https is a DNS record, a cert and
/// an nginx change away, not a manifest edit. So the hash is not defence in depth here, it
/// is the defence. <see cref="ClientAcquirer"/> refuses an entry without one.
/// </summary>
public sealed class ClientArchive
{
    ...
    /// <summary>
    /// Expected size. Load-bearing three times over: the disk-space check, the progress bar,
    /// and — since this download resumes — the offset a broken transfer restarts from.
    ///
    /// ⚠ It must be the host's real Content-Length, not an approximation. New-Manifest.ps1
    /// writes the MEASURED length for exactly this reason; a hand-typed one that is wrong
    /// fails every install at the length check before the hash is ever reached.
    /// </summary>
    [JsonPropertyName("bytes")] public long Bytes { get; set; }

    /// <summary>
    /// SHA-256 of the archive as the host serves it. Generated by New-Manifest.ps1 from the
    /// bytes it downloads, never typed by hand.
    /// </summary>
    [JsonPropertyName("sha256")] public string Sha256 { get; set; } = "";
}
...
public sealed class ManifestArchive
{
    ...
    /// <summary>
    /// ⚠ LA-14. Published and deserialised, but NOTHING READS IT.
    /// <see cref="SyncService.SyncArchivesAsync"/> verifies these by hash alone — deliberately,
    /// since a length is not evidence about content. Kept for legibility of the manifest, not
    /// as a check; anything relying on it is relying on nothing today.
    /// </summary>
    [JsonPropertyName("size")] public long Size { get; set; }
