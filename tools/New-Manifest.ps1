...
param(
    ...
    # Addons the launcher DELETES from the client, folder and saved variables both, on the
    # next sync. Stronger than -ForceDisableAddOns and not a substitute for it: keep a name in
    # BOTH, because a client running a launcher older than 1.14.0 ignores this list entirely
    # and the tick is all that is holding the addon back there.
    #
    # ** Only for an addon we shipped and have retired. This is not a way to police what
    # players install - an addon the player chose is removed only by a REPAIR they pressed and
    # confirmed. A name that also appears in the payload, in archives.json or in
    # forceEnableAddOns is rejected below rather than published, because a manifest that both
    # installs and deletes the same addon would fight itself on every launch forever.
    [string[]]$RemoveAddOns = @('QuestHelper'),
    ...
    # The base client plus the byte patches the launcher applies to it. Generated by
    # Build-ClientPatch.py -- never hand-written, because the result hash it carries is only
    # meaningful if the same tool computed it from the same patch list. Delete the file to
    # publish no clientPatch block at all, which is the rollback.
    [string]$ClientPatch   = 'C:\Wotlk\Launcher\tools\client-patch-block.json',
        ...
        foreach ($e in $external) {
            ...
            switch -Regex ($ext) {
                '^\.(exe|dll|dat)$' {
                    ...
                    # UncappedBase.dat is included because the derived client inherits its
                    # header bits: a base without the flag produces a runnable client without
                    # it, and the invariant this check defends is about the file that runs.
                    $peName = [IO.Path]::GetFileName($e.path)
                    if ($peName -ieq 'UncappedClient.dat' -or $peName -ieq 'UncappedBase.dat' -or $peName -ieq 'Wow.exe') {
...
# --- The derived client: base pin plus byte patches ---------------------------------------
# The runnable client is BUILT ON THE PLAYER'S MACHINE, from the base file pinned above plus
# the pokes in this block, rather than published whole.
#
# Why, in one sentence: SyncService.IsCurrentAsync re-hashes every manifest file against its
# pin on every launch, so a launcher that edited the installed client would find it mismatched
# and re-download it every launch, forever. Splitting the pristine base (a normal pinned file
# the sync verifies and nothing ever touches) from the patch list (data, in this block) is what
# makes local patching safe -- and it is why a client-side hex fix now costs a manifest edit
# and ZERO bytes of client download instead of 7.7 MB per player.
#
# Absent block == feature off == the launcher runs whatever UncappedClient.dat it finds,
# exactly as every release before this one did. That is the rollback, and it is one file.
#
# Generated by Build-ClientPatch.py; never hand-written. The result hash in particular is the
# whole guarantee and is only meaningful if the same tool computed it from the same patch list.
$clientPatchEntry = $null
if (Test-Path $ClientPatch) {
    $cpDoc = Get-Content $ClientPatch -Raw | ConvertFrom-Json
    $cp = $cpDoc.clientPatch

    if (-not $cp) { throw "$ClientPatch has no 'clientPatch' object. Re-run Build-ClientPatch.py." }

    Write-Host "`nDerived client:" -ForegroundColor Cyan

    foreach ($required in 'basePath', 'outputPath', 'baseSha256', 'resultSha256') {
        if (-not $cp.$required) { throw "clientPatch block has no '$required'." }
    }
    if (@($cp.patches).Count -eq 0) { throw "clientPatch block carries no patches." }

    # THE STALE-BLOCK CHECK. The base hash here must be the hash of the file we are actually
    # publishing as the base. Without this, editing external-files.json to point at a new base
    # while forgetting to re-run Build-ClientPatch.py would publish a patch list cut from a
    # file nobody downloads -- every player's build would refuse at the first expect check and
    # PLAY would go off realm-wide.
    $baseEntry = @($files | Where-Object { $_.path -ieq $cp.basePath })
    if ($baseEntry.Count -ne 1) {
        throw "clientPatch.basePath '$($cp.basePath)' is not among the published files. The base must be a normal pinned manifest entry."
    }
    if ($baseEntry[0].sha256 -ne $cp.baseSha256) {
        throw @"
clientPatch.baseSha256 does not match the file being published as $($cp.basePath).

  published : $($baseEntry[0].sha256)
  block says: $($cp.baseSha256)

The patch list was cut from a different base. Re-run Build-ClientPatch.py against the file
that is actually pinned in external-files.json. Nothing was written.
"@
    }

    # And the DERIVED client must not be a manifest file. This is the re-download loop the
    # whole design exists to avoid, so it is asserted rather than left to care.
    if (@($files | Where-Object { $_.path -ieq $cp.outputPath }).Count -gt 0) {
        throw "$($cp.outputPath) is BOTH published as a file and derived by clientPatch. The sync would re-download it on every launch. Remove it from external-files.json."
    }

    $patchList = @()
    foreach ($p in $cp.patches) {
        foreach ($required in 'id', 'offset', 'expect', 'write') {
            if (-not $p.$required) { throw "a clientPatch entry has no '$required'." }
        }
        if ($p.expect -notmatch '^[0-9A-Fa-f]+$') { throw "patch '$($p.id)': expect is not hex." }
        if ($p.write -notmatch '^[0-9A-Fa-f]+$') { throw "patch '$($p.id)': write is not hex." }
        if ($p.expect.Length % 2 -ne 0) { throw "patch '$($p.id)': expect is not a whole number of bytes." }
        # Length-preserving is load-bearing: anything else would move every later offset.
        if ($p.expect.Length -ne $p.write.Length) {
            throw "patch '$($p.id)': expect is $($p.expect.Length / 2) bytes but write is $($p.write.Length / 2)."
        }
        ...
        Write-Host ("  + {0,-28} {1} {2,4} bytes" -f $p.id, $p.offset, ($p.write.Length / 2)) -ForegroundColor Green
    }

    $clientPatchEntry = [ordered]@{
        basePath     = $cp.basePath
        outputPath   = $cp.outputPath
        baseSha256   = $cp.baseSha256
        resultSha256 = $cp.resultSha256
        size         = $cp.size
        patches      = @($patchList)
    }

    Write-Host "  base   $($cp.basePath) $($cp.baseSha256.Substring(0,12))..." -ForegroundColor DarkGray
    Write-Host "  result $($cp.outputPath) $($cp.resultSha256.Substring(0,12))..." -ForegroundColor DarkGray
}
else {
    Write-Host "`nDerived client: no $ClientPatch - no clientPatch block written." -ForegroundColor Yellow
    Write-Host "  Clients will run whatever UncappedClient.dat they have, as before." -ForegroundColor DarkGray
}
...
$manifest = [ordered]@{
    ...
    # The base client and the pokes that turn it into the runnable one, or absent to leave
    # every client alone. See the block that builds it for why this is not a files[] entry.
    clientPatch        = $clientPatchEntry
    ...
    # ** DELETED from Interface\AddOns, along with their saved variables, on the next sync.
    # This is the launcher's one standing exception to "never delete an addon we did not
    # write", and it exists because forceDisableAddOns turned out to be only half a retirement:
    # the folder stays, the saved variables stay, and one tick of a checkbox brings it all
    # back. Requires launcher 1.14.0; older clients ignore the field and keep the addon
    # unticked, which is the same behaviour they have today.
...
# --- removeAddOns cannot name something we also install -----------------------------------
# The launcher deletes these on every sync with no dialog in front of it, so a name that also
# appears in the payload, in an archive or in forceEnableAddOns would have the sync installing
# and deleting the same addon on every launch, forever, on every client at once.
#
# AddOnRemover refuses that case at runtime too -- this is the same rule, enforced where it can
# still be fixed by editing a parameter rather than by shipping a new manifest.
if (@($manifest.removeAddOns).Count -gt 0) {
    $installedFolders = New-Object 'System.Collections.Generic.HashSet[string]' ([StringComparer]::OrdinalIgnoreCase)

    foreach ($f in $manifest.files) {
        $parts = $f.path -split '/'
        for ($i = 0; $i -lt $parts.Count - 1; $i++) {
            if ($parts[$i] -eq 'AddOns') { [void]$installedFolders.Add($parts[$i + 1]); break }
        }
    }
    foreach ($a in $manifest.archives)          { [void]$installedFolders.Add($a.name) }
    foreach ($n in $manifest.forceEnableAddOns) { [void]$installedFolders.Add($n) }

    $conflicts = @($manifest.removeAddOns | Where-Object { $installedFolders.Contains($_) })
    if ($conflicts.Count -gt 0) {
        throw ("removeAddOns names {0} addon(s) this manifest also installs: {1}. " -f
               $conflicts.Count, ($conflicts -join ', ')) +
              "Remove them from -RemoveAddOns, or stop shipping them, but not both."
    }

    Write-Host ("`nRetired addons: {0} will be DELETED from clients ({1})." -f
                @($manifest.removeAddOns).Count, ($manifest.removeAddOns -join ', ')) -ForegroundColor Yellow
}
...
# ** removeAddOns is normally a list of ONE, which is precisely the collapse case above. A
# bare string here does not degrade gracefully: the launcher parses it into List<string> and a
# throw at that point rejects the ENTIRE manifest, so every player would be told the update
# server is unreachable while the generator reported success.
Assert-JsonArray -Text $written -Name 'removeAddOns'       -Parsed $check.removeAddOns       -Expected @($RemoveAddOns).Count
...
# Same shape of check for the derived client, and it matters more: a clientPatch block that
# lost its patch list to array collapse would tell every launcher to build a client from zero
# patches, hash it, and refuse the result -- PLAY off, realm-wide, on a manifest that looked
# like it published cleanly.
if ($clientPatchEntry) {
    if ($check.clientPatch.resultSha256 -ne $clientPatchEntry.resultSha256) {
        throw "Wrote $OutFile but its clientPatch result hash is '$($check.clientPatch.resultSha256)', expected '$($clientPatchEntry.resultSha256)'."
    }
    Assert-JsonArray -Text $written -Name 'patches' -Parsed $check.clientPatch.patches -Expected $clientPatchEntry.patches.Count
}
if (-not $clientPatchEntry -and $check.clientPatch) {
    throw "Wrote $OutFile but it carries a clientPatch block that should not be there."
}
